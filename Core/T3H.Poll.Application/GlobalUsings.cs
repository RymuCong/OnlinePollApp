global using T3H.Poll.Domain.Entities;
global using T3H.Poll.Application.Common.Commands;
global using T3H.Poll.Application.Queries;
global using T3H.Poll.Domain.Events;
global using Microsoft.Extensions.DependencyInjection;
global using System.Reflection;
global using T3H.Poll.Domain.Repositories;
global using T3H.Poll.CrossCuttingConcerns.ExtensionMethods;
global using T3H.Poll.CrossCuttingConcerns.Exceptions;
global using T3H.Poll.Application.Common.Services;
global using T3H.Poll.Application.Common;
global using T3H.Poll.Domain.Enums;
global using T3H.Poll.Application.CommonSetting.DTOs;
global using  T3H.Poll.Application.Users.DTOs;
global using T3H.Poll.Application.Common.DTOs;
global using AutoMapper;

